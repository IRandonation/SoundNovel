...
- **`utils/`**:
  - [multi_model_client.py](file:///d:/Project/SoundNovel/novel_generator/utils/multi_model_client.py): 一个强大的客户端，用于与各种 LLM 提供商（豆包、DeepSeek、Ark）进行交互。处理模型路由、重试和降级。
...
- **命令行界面 (CLI)**: 用于运行生成流水线（初始化、大纲、扩写）的命令行接口。
  - [main.py](../novel_generator/cli/main.py): CLI 主入口
  - [agent/cli_repl.py](../novel_generator/agent/cli_repl.py): Agent 对话模式 REPL
- **脚本 (`05_script/`)**:
...
### 多模型策略
系统支持为不同任务切换不同的模型：
- **逻辑/规划**: 使用“长文本”模型（如 豆包）进行高层级的大纲规划和一致性检查。
- **正文创作**: 使用更快速、更具成本效益的模型（如 DeepSeek）进行正文扩写。
