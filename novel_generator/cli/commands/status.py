...
def run(args: argparse.Namespace) -> int:
    ...
    print_info("--- API 配置 ---")
    provider_names = {"doubao": "豆包/火山引擎", "deepseek": "DeepSeek"}
    print(
        f"  🤖 当前服务商: {provider_names.get(status['api_provider'], status['api_provider'])}"
    )
    ...
    if total > 0:
        ...
        if last_draft < total:
            ...
            print()
            print_info(
                f"💡 续写建议: 运行 'soundnovel continue' 从第 {next_chapter} 章继续"
            )
        else:
            print()
