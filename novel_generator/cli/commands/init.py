...
def run(args: argparse.Namespace) -> int:
    project_root_path = Path(args.project_root).resolve() if args.project_root else Path.cwd()
    ...
    try:
        ...
        manager = ProjectManager(project_root_path)
        interactive = not args.skip_config
        
        if manager.initialize_project(interactive=interactive):
            print_success("项目初始化完成！")
            print()
            print_info("📋 下一步操作指南:")
            print("  1. 填写 01_source/core_setting.yaml - 小说核心设定")
            print("  2. 填写 01_source/overall_outline.yaml - 整体大纲")
            print("  3. 运行 'soundnovel outline' 生成章节大纲")
            return 0
        else:
            print_error("项目初始化失败")
            return 1
