...
def setup_cli_logging(log_file: str = ".logs/cli.log") -> logging.Logger:
    """
    为CLI命令设置日志
...
def get_config_manager(project_root: Optional[str] = None, novel_id: Optional[str] = None):
    """
    获取配置管理器实例

    Args:
        project_root: 项目根目录（默认为当前目录）
        novel_id: 小说ID（可选，用于多小说管理）
    ...
    return ConfigManager(root, novel_id=novel_id)
