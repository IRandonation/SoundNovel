...
class EmotionalArcTracker:
    ...
    def analyze_chapter(
        ...
        if not emotion_scores:
            ...
            primary_emotion = max(
                emotion_scores.keys(), key=lambda e: emotion_scores[e]
            )
            intensity = min(
                10,
                self.EMOTION_SPECTRUM[primary_emotion]["intensity_base"]
    ...
    def load_tracking_file(self, tracking_path: str) -> bool:
        ...
        try:
            ...
            if not tracking_file.exists():
                self.logger.info(
                    f"情感弧线追踪文件不存在，将创建新文件: {tracking_path}"
                )
                return False
            ...
            if not data or "情感弧线" not in data:
                return False

            arc_data = data["情感弧线"]
            self.current_arc = arc_data.get("当前状态", "上升")
            self.arc_start_chapter = arc_data.get("起点章节", 1)

            beats_data = arc_data.get("节拍记录", [])
            for beat_item in beats_data:
                beat = EmotionalBeat(
                    chapter=beat_item.get("章节", 0),
                    emotion=beat_item.get("情感", "平静"),
                    intensity=beat_item.get("强度", 5),
                    event=beat_item.get("事件", ""),
                    character=beat_item.get("角色", ""),
                )
                self.beats.append(beat)

            self.logger.info(f"从追踪文件加载了 {len(self.beats)} 个情感节拍")
            return True

        except Exception as e:
            self.logger.error(f"加载情感弧线追踪文件失败: {e}")
            return False
