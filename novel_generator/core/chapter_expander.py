...
"""
    ...
    def _validate_content(self, content: str, chapter_outline: Dict[str, Any]) -> str:
        ...
        if word_count < target_count * 0.8:
            self.logger.warning(f"内容字数不足：{word_count} < {target_count}")
        elif word_count > target_count * 1.2:
            self.logger.warning(f"内容字数过多：{word_count} > {target_count}")
                ...
                if str(f"第{chapter_num}章") in outline:
                    ...
                    if chapter_num > context_window:
                        ...
                        if old_chapter >= chapter_range[0]:
                            old_content_file = output_dir / f"chapter_{old_chapter:02d}.md"
                    ...
                    # 添加新章节到上下文
                    new_content_file = output_dir / f"chapter_{chapter_num:02d}.md"
