...
DEFAULT_CONFIG = {
    ...
    "providers": {
        "zhipu": {
            ...
            "default_models": ["glm-4-long", "glm-4.5-flash", "glm-4"]
        },
        "deepseek": {
            "name": "DeepSeek",
        ...
        "doubao": {
            ...
            "default_models": ["doubao-1-5-lite-32k-250115"]
        }
    },
    "quality_check": {
        "banned_words": [
            "复杂的思绪", "难以言喻", "命运的齿轮",
            "心中五味杂陈", "一种莫名的", "仿佛", "似乎", "这一刻"
        ],
        "ai_patterns": [
            "一种莫名的", "仿佛", "似乎", "这一刻", "不由得", "不禁"
        ]
    }
}
...
class GenerationConfigManager:
    ...
    def set_role_config(self, role_name: str, **kwargs) -> bool:
        ...
        if role_name not in self._config["roles"]:
            self._config["roles"][role_name] = copy.deepcopy(DEFAULT_CONFIG["roles"].get(role_name, {}))
